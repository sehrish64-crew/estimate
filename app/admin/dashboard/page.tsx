...
export default function AdminDashboard() {
      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star
            key={star}
            className={`w-5 h-5 ${
              star <= rating ? 'fill-[#D97706] text-[#D97706]' : 'text-gray-300'
            }`}
          />
        ))}
      </div>
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-[#E8B899] text-[#B45309] border border-[#E8B899]">Pending</span>
      case 'processing':
      ...
      case 'cancelled':
        return <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-orange-100 text-orange-800 border border-orange-200">Cancelled</span>
      case 'refunded':
                      ...
                      <div className="flex gap-1 md:gap-2 flex-wrap">
                        {pendingTab ? (
                          <>
                            ...
                            <Button onClick={() => handleReject(review.id)} variant="outline" className="text-[#D97706] border-[#E8B899] hover:bg-[#F5ECDA]" size="sm"><span className="hidden md:inline">Reject</span></Button>
                          </>
                        ) : (
                          <Button onClick={() => handleDelete(review.id)} variant="outline" className="text-orange-600 border-orange-200 hover:bg-orange-50" size="sm"><Trash2 className="w-3 md:w-4 h-3 md:h-4 mr-1" /><span className="hidden md:inline">Delete</span></Button>
                        )}
                      </div>
                    </td>
                  </tr>
